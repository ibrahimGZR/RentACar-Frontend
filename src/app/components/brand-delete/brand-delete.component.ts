...
export class BrandDeleteComponent implements OnInit {
  ...
  getByBrandId(brandId:number){
    this.brandService.getBrandById(this.activatedRoute.snapshot.params["brandId"]).subscribe(response=>{
  ...
  delete(){
    if (this.brand) {
      this.brandService.deleteBrand(this.brand).subscribe(response=>{
