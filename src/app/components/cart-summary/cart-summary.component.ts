...
export class CartSummaryComponent implements OnInit {
  ...
  getCards() {
    this.paymentService.getPayments().subscribe((response) => {
      this.cards = response.data.filter(
        (p) => p.userId == parseInt(this.localStorageService.get('userId'))
      );
    });
  }
  getCar() {
    this.carService
      .getCarDetailsById(parseInt(this.localStorageService.get('carIdCart')))
      .subscribe((response) => {
  ...
  paymentForRent() {
    this.userService.getUsers().subscribe((response) => {
      ...
      this.users.forEach((user) => {
        if (
          user.email == this.localStorageService.get('email') &&
          this.checkIfUserFindexPoint
        ) {
          this.user = user;
  ...
  AddRental() {
    ...
    rentalModel.customerId = parseInt(
